...
class TestBuildAgent:
    ...
    async def test_build_agent_falls_back_when_mcp_unavailable(self):
        ...
        with patch("agent.graph.MultiServerMCPClient") as MockClient, \
...
class TestRunAgent:
    ...
    async def test_run_agent_returns_string(self):
        ...
        mock_agent.ainvoke = AsyncMock(return_value={
            "messages": [MagicMock(content="Final answer")]
        })

        with patch("agent.graph.recall_memories", return_value=[]), \
    ...
    async def test_run_agent_injects_memories(self):
        ...
        with patch("agent.graph.recall_memories", return_value=["User is Alice"]), \
             patch("agent.graph._maybe_save_memory"):
            await run_agent(
                agent=mock_agent,
